...
```
Prometheus uses Kubernetes API to get all metrics, therefore do we need to create `RBAC` policy with `read access` to the required API groups and bind the policy to the `monitoring` namespace.

Create the policy manifest, `monitor-clusterRole.yaml`.
...
## Create Config Map
The config map externalizes Prometheus configuration to make it possible to access and change the settings without having to rebuild the Prometheus image for each change.

create `monitoring-config-map.yaml`:
```
apiVersion: v1
kind: ConfigMap
metadata:
